...
def show_iiv_distance(
    ...
    if show_cluster:
        base_dir = os.path.dirname(out_img)
        base_name = os.path.basename(out_img).split(".")[0]
    # Only show grp
        emo_embGrp = separete_emo(emo_labels, emo_embs, grp_labels)
        for emo, emb_grp in emo_embGrp.items():
            embs, grps = emb_grp
            png_f = base_dir + "/{}_c{}.png".format(base_name, emo)
            show_emb_grp_distr(embs, grps, png_f)
...
def separete_emo(emos, embs, grps):
    emo_embGrp = {}
    emo_sets = list(set(emos))
    for emo in emo_sets:
        emo_mask = emos == emo
        embs_emo= embs[emo_mask]
        grps_emo = grps[emo_mask]
        emo_embGrp[emo] = [embs_emo, grps_emo]
    return emo_embGrp


def show_emb_grp_distr(embs, grps, png_f, fig_size=8):
    fig, ax = plt.subplots(figsize=(fig_size, fig_size))
    tSNE_metrics = TSNE(n_components=2, random_state=0).fit_transform(embs)

    grp_sets = list(set(grps))
    scatters = []
    legends = []
    for grp in grp_sets:
        grp_mask = grps == grp
        tsne_grp = tSNE_metrics[grp_mask]
        scatter = ax.scatter(tsne_grp[:, 0], tsne_grp[:, 1], c=color_cluster[grp], s=12)
        scatters.append(scatter)
        legends.append(grp)
    ax.legend(
        scatters,
...
def show_att_emo_grp_distr(
    ...
    legend_names = []
    scatters = []
    for emo, grp_att in emo_att_stat_dict.items():
        for grp, att_mean_sd in grp_att.items():
            att_mean, att_sd = att_mean_sd
            scatter = ax.scatter(att_mean, att_sd, c=colors[emo],
                                 marker=makers[grp], s=12)
            legend_names.append("emo:{}, grp:{}".format(emo, grp))
            scatters.append(scatter)

    ax.legend(scatters,
              legend_names,
              loc="lower left", title="Legend", markerscale=3, fontsize=12,
              bbox_to_anchor=(0.85, 0))
    fig.savefig(out_png, dpi=300)


def show_att_emo_grp_distr_sub(
        emo_att_stat_dict,
        out_png="output.png",
        col_n=2,
        fig_size=8
):
    emo_n = len(emo_att_stat_dict.keys())
    row_n = int(emo_n / col_n) + 1
    plt.subplots_adjust(wspace=0.3, hspace=0.4)
    fig, ax = plt.subplots(row_n, col_n, figsize=(fig_size, fig_size))

    for i, emo in enumerate(emo_att_stat_dict.keys()):
        legend_names = []
        scatters = []
        r = int(i / col_n)
        c = int(i % col_n)
        for g in emo_att_stat_dict[emo].keys():
            att_mean, att_sd = emo_att_stat_dict[emo][g]
            # each scatter group rendering in subplot
            scatter = ax[r, c].scatter(att_mean, att_sd, marker=makers[g])
            scatters.append(scatter)
            legend_names.append("G{}".format(g))
        # subplot attribute
        ax[r, c].legend(
...
if __name__ == '__main__':
    iiv_dir_base = "/home/rosen/project/FastSpeech2/preprocessed_data/ESD/{}"
    iiv_inter_only_dir = "/home/rosen/project/FastSpeech2/ESD/iiv_reps_wrong"
    iiv_anchorFalse_02_02_w10_net = iiv_dir_base.format("iiv_anchorFalse_02_02_w10_net")
    iiv_anchorFalse_02_02_w05_net = iiv_dir_base.format("iiv_anchorFalse_02_02_w05_net")
    iiv_anchorFalse_02_02_w10 = iiv_dir_base.format("iiv_anchorFalse_02_02_w10")
    iiv_anchorFalse_02_02_w07 = iiv_dir_base.format("iiv_anchorFalse_02_02_w07")

    ssl_dir = "/home/rosen/project/FastSpeech2/ESD/emo_reps"
    psd_dir = "/home/rosen/project/FastSpeech2/ESD/psd_reps"
    emo_dict_f = "/home/rosen/project/FastSpeech2/ESD/metadata_22322.json"
    maxPoint = -1
    ...
    if True:
        for name, emb_dir in zip(["iiv"], [iiv_anchorFalse_02_02_w05_net]):
            show_iiv_distance(
                emb_dir,
                emo_dict_f,
                "exp/{}_emb_{}.png".format("iiv_anchorFalse_02_02_w05_net", maxPoint),
                maxPoint,
                20
            )
